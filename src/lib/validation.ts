// Lightweight, dependency-free field validators for the apply funnel.
// Each returns an error string, or undefined when the value is valid.
...
export const accepted =
  (label = "This consent"): Validator =>
  (v) =>
    v === "true" ? undefined : `${label} must be accepted.`;
...
export const ssn: Validator = (v) =>
  digits(v).length === 9 ? undefined : "Enter a valid 9-digit SSN or ITIN.";
...
export const routing: Validator = (v) => {
  ...
  if (d.length !== 9) {
    return "Routing numbers are exactly 9 digits.";
  }
