...
import type { AdminApplication } from "@/src/lib/type";
...
export default function AdminApplicationsPage() {
  ...
  const [timeInBusiness, setTimeInBusiness] = useState(
    searchParams.get("time_in_business")?.trim() || "",
  );
  const [fundingTimeline, setFundingTimeline] = useState(
    searchParams.get("funding_timeline")?.trim() || "",
  );
  const [monthlyRevenue, setMonthlyRevenue] = useState(
    searchParams.get("monthly_revenue")?.trim() || "",
  );
        ...
        const data = await api.getApplications({
          ...
          date:
            ...
            monthlyRevenue
              ? undefined
              : filterDate,
          time_in_business: timeInBusiness || undefined,
          funding_timeline: fundingTimeline || undefined,
  ...
  useEffect(() => {
    ...
    if (timeInBusiness) {
      params.set("time_in_business", timeInBusiness);
    }
    if (fundingTimeline) {
      params.set("funding_timeline", fundingTimeline);
    }
    if (monthlyRevenue) {
      params.set("monthly_revenue", monthlyRevenue);
    }
    ...
    <div>
      ...
      <div className="flex items-center gap-2 mt-6 flex-wrap">
        ...
        <select
          ...
          onChange={(e) => {
            setFundingTimeline(e.target.value);
            setPage(1);
          }}
          className="rounded-lg border border-gray-300 px-3 py-2 text-sm"
        >
        ...
        >
          ...
          <option value="8000_25000">$8,000 – $25,000</option>
          <option value="25000_75000">$25,000 – $75,000</option>
        ...
        <div>
          <button
            type="button"
            onClick={handleReset}
            className="px-4 py-1.5 text-primary border border-gray-300 rounded-md text-sm transition hover:bg-primary/5 cursor-pointer flex items-center justify-center"
          >
            <LucideRefreshCcw
              size={20}
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                rows.map((loan) => (
                  <tr
                    key={loan.id ?? loan.application_id}
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-navy-700">{loan.email}</div>
                      {loan.phone && (
                    ...
                    <td className="px-4 py-3 text-navy-800 tabular-nums">
                      {formatUSD(Number(loan.funding_amount)) || "—"}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-navy-700">
                      {formatDate(loan.createdAt)}
                    </td>
                    <td
                      onClick={() => handleGoToPage(loan.application_id)}
                      className="cursor-pointer whitespace-nowrap px-4 py-3 text-navy-700"
                    >
